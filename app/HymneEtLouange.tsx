...
const HymneEtLouange: React.FC = () => {
  ...
  // Gère le défilement rapide vers le haut ou le bas
  const scrollToTop = () => {
  ...
  // Gère la visibilité des flèches lors du défilement
  const handleScroll = (event: any) => {
    const offsetY = event.nativeEvent.contentOffset.y;
    setShowUpArrow(offsetY > 100); // Affiche la flèche vers le haut après un certain défilement
    setShowDownArrow(offsetY < HymneEtLouangeSongs.length * 60 - 600); // Affiche la flèche vers le bas si pas tout en bas
  };
      ...
      <FlatList
        ...
        data={
          searchInput.trim() === ""
            ...
            ? HymneEtLouangeSongs.filter(
                (song) => song.id.toString() === searchInput.trim()
              )
            : HymneEtLouangeSongs.filter((song) => {
        ...
        renderItem={({ item, index }) => (
          <RenderItem
            item={item}
            onPress={() => handleChantPress(item, index)}
          />
        )}
        keyExtractor={(item) => item.id.toString()} // Chaque clé est une chaîne
...
const styles = StyleSheet.create({
  container: {
    ...
    padding: scale(16), // Ajuste la marge en fonction de l'échelle
  },
  item: {
  ...
  songItem: {
    backgroundColor: "#ffffff", // Blanc pour le fond de chaque item
    padding: scale(16), // Marge responsive
    marginVertical: verticalScale(8), // Marge verticale responsive
    marginHorizontal: scale(12), // Marge horizontale responsive
    borderRadius: scale(8), // Border-radius ajusté
    elevation: 3,
    shadowColor: "#000",
    shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.5,
    shadowRadius: 4,
    borderColor: "red",
    borderLeftWidth: scale(3), // Ajuste la largeur de la bordure
    borderRightWidth: scale(2), // Ajuste la largeur de la bordure
  },
  songTitle: {
    fontSize: scale(15), // Taille de police responsive
    fontWeight: "600",
    color: "#0A1E42",
  },
  upButton: {
